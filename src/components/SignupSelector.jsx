...
const SignupSelector = () => {
  const navigator = useNavigate(); // Corrected naming

  const handleSubmit = (userType) => {
    if (userType === 'student') {
      navigator("/student/signup");
    } else if (userType === 'lecturer') {
      navigator("/lecture/signup");
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-100">
      {/* Back Button */}
      <button
        onClick={() => navigator(-1)} // Fixed from navigate(-1)
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-96">
        <h2 className="text-2xl font-bold mb-4 text-center">Signup As</h2>
